...
private fun collectLlvmModules(context: Context, generatedBitcodeFiles: List<String>): LlvmModules {
    ...
    val (runtimeModules, additionalModules) = listOf((runtimeNativeLibraries + bitcodePartOfStdlib), additionalBitcodeFiles)
            .map {
                it.map { bitcodeFile ->
    ...
    return LlvmModules(
            runtimeModules + context.generateRuntimeConstantsModule(),
